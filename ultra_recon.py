...
def main():
    ...
    target = args.target
    if args.image == "nmap":
        o = client.containers.run(args.image, command=target + " -p 80")
    if args.image == "pydnsrecon":
        o = client.containers.run(args.image, command=target, environment=["censys_API_ID=" + censys_API_ID, "censys_secret=" + censys_secret])
